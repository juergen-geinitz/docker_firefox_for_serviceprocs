...
${DOCKER} run -it --rm -P --user "0:0"  --volume ${HOME}/tmp:/home/user:rw --volume ${HOME}:/home:rw --env HOME=/tmp --env DISPLAY=:0 --volume /etc/localtime:/etc/localtimeXX:ro  --volume /etc/timezone:/etc/timezoneXX:ro --privileged  --ipc host --name bbb --network host --no-hosts --pid host --security-opt label=disable --ulimit host --userns keep-id localhost/bladecenter bash -l
